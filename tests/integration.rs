...
#[test]
fn daemon_socket_serves_status_and_session_control() {
    let dir = unique_tmp_dir("daemon_socket");
    fs::write(dir.join("f.rs"), "original").unwrap();
    run(&dir, &["init"]);

    let (serve_code, serve_out, serve_err) = run(&dir, &["serve", "--daemon"]);
    assert_eq!(
        serve_code, 0,
        "serve --daemon failed: {serve_out}{serve_err}"
    );

    let mut status_ready = false;
    let mut last_status = String::new();
    for _ in 0..20 {
        let (status_code, status_out, _) = run(&dir, &["status"]);
        last_status = status_out;
        if status_code == 0 && last_status.contains("daemon:   running") {
            status_ready = true;
            break;
        }
    ...
    assert!(
        status_ready,
        "status should eventually report daemon socket availability: {last_status}"
    );
    ...
    let start = Command::new(bin_path())
        .args([
            ...
            "codex",
            "--metadata",
            r#"{"prompt":"refactor auth","tool_name":"Write"}"#,
        ])
        .current_dir(&dir)
        .output()
        .unwrap();
    ...
    let active = dir.join(".agent-undo/active-session.json");
    assert!(active.exists(), "session start should create active marker");
    ...
    let end = Command::new(bin_path())
        ...
        .unwrap();
    assert_eq!(end.status.code().unwrap_or(-1), 0);
    assert!(!active.exists(), "session end should clear active marker");

    let (_, sessions, _) = run(&dir, &["sessions"]);
    assert!(
        sessions.contains("codex"),
        "session should be recorded through daemon IPC: {sessions}"
    );
